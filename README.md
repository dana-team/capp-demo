...
A small, fake food-ordering app used to demo [Capp](https://github.com/dana-team/container-app-operator) features: scale-to-zero autoscaling, native Kafka/cron eventing (no client boilerplate), custom TLS routes, and centralized log shipping to Elastic.
...
|---|---|---|
| `frontend` | React (Vite) + nginx | order form, calls `orders-service` directly from the browser |
...
- `container-app-operator` installed on the cluster, with a `danaeats` namespace (or pass `--create-namespace`)
...
```bash
for svc in frontend orders-service kitchen-service notifications-service; do
  docker build -t ghcr.io/YOUR_ORG/danaeats-$svc:latest services/$svc
  docker push ghcr.io/YOUR_ORG/danaeats-$svc:latest
done
```
...
```bash
helm install danaeats chart/ --namespace danaeats --create-namespace
```

## Demo script

1. `kubectl get capps -n danaeats` - all 4 Ready, note the `Custom URL` / `AutoScale Type` columns.
2. Open the frontend URL, place an order - watch it flow through Kafka into `kitchen-service` then `notifications-service`, all visible as JSON log lines in Kibana filtered by `orderId`.
3. Stop sending traffic, watch `frontend`/`orders-service` scale to zero (`kubectl get pods -n danaeats -w`); send traffic again and observe the cold start.
